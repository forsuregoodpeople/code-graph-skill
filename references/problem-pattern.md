...
Render:  Dashed red edges between all nodes in the cycle
Badge:   🔴 Circular
...
Render:  Red border on node
Badge:   ⚠️ Deep nest (N levels)
...
Render:  Yellow warning overlay on node
Badge:   ⚠️ Unhandled async
...
Render:  Orange edge from function to module scope node
Badge:   ⚠️ Shared mutable state
...
Badge:   ⚠️ High fan-out (N imports)
...
Render:  Node size scales up; marked as "Core" (blue) or "Risk" (red if >150 LOC)
Badge:   ℹ️ Core module (N dependents)
...
| Badge | Color | Meaning |
|---|---|---|
| 🔴 Critical | Red node | Fix before shipping |
| ⚠️ Warning | Yellow/orange | Fix in next refactor sprint |
| ℹ️ Info | Blue overlay | Worth knowing, not urgent |
| ✅ OK | Green | Intentionally good pattern |

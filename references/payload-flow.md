...
```txt
PAYLOAD FLOW -- POST /users (register)

  [REQUEST]  { name, email, password, isAdmin }            <- req.body (no validation) [!] UNVALIDATED_PAYLOAD
       |
       |  (passed whole to service)
       v
  [SERVICE: UserService.create()]
       |  +passwordHash: string  (bcrypt)   password: raw=>hashed [SENSITIVE]
       |  (whole body spread into entity)
       v
  [PERSIST: usersRepo.insert()]  *           [!!] OVERPOSTING -- isAdmin from body persisted
       |  shape: { name, email, password[SENSITIVE], passwordHash[SENSITIVE], isAdmin }
       v
  [RESPONSE: 201 user]                       [!!] LEAKED_FIELD -- password, passwordHash in body
